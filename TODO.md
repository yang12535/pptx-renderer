...
- [x] 重构 `.slide` 为绝对定位堆叠布局（脱离 display:none/block 模式）
- [x] 预置 CSS 过渡类：`fade` / `slide-left` / `slide-right` / `slide-up` / `zoom` / `flip`
...
- [x] 创建 `animations.css`，预置核心 `@keyframes`
  - [x] `fadeIn`
  - [x] `slideInUp`
  - [x] `slideInLeft` / `slideInRight`
  - [x] `zoomIn`
  - [x] `growIn`（从低往高，scaleY）
  - [x] `expandRight`（从左向右，scaleX）
- [x] 修改 `html-render.js`，给 `renderElement` 增加 `animClass` 与 `animDelay`
- [x] 实现「幻灯片激活时子元素依次入场」机制
  - [x] CSS 纯方案：`.slide.active .p-el` 控制 `animation-play-state`
  - [x] 备选 JS 方案：手动 reflow 重启动画
- [x] 支持按元素索引自动错峰（默认 100ms 间隔）
- [x] 预留 PPTX 原生动画解析接口（不实现，仅留扩展点）
...
- [x] 幻灯片切换时重置内部元素动画（避免二次进入无动画）
- [x] 触摸手势支持（滑动切页）
- [x] 全屏模式（Fullscreen API）
- [x] 进度指示器（底部进度条）
- [ ] 添加「自动播放」模式（interval 5s）
- [ ] 缩略图栏渲染实际内容（当前为 DOM 克隆，需优化）
