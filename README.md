...
- 入场动画：幻灯片元素错峰进入，表格逐行显现，柱状图弹出，折线图按年份逐点展开，并且不跟随系统 reduced-motion 设置自动关闭。
...
推荐主流程是直接打开上传模式页面。页面只加载 `dist/assets/` 下随包分发的本地组件，用户在浏览器中选择本机 `.pptx` 后，由浏览器端 JSZip 和解析器完成读取、解析和展示，PPTX 文件不需要先上传到服务端。
